...
- Cycle operations now revalidate owner/resource scope in the same RLS transaction. New cycles on archived fields are rejected by both the application and the restricted database role; existing history stays readable. Client-supplied catalog snapshots, journal/outbox, idempotent mutation protocols and the full lifecycle policy for existing cycles remain pending.
