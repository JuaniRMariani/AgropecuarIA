...
- Tooling: scripts de clúster efímero y fixtures, separados de runtime productivo.
...
- API → pool/DB: SQL parametrizado dentro de transacción; `set_config(..., true)` antes de datos tenant.
...
2. Miembro A envía organization/record ID de B → API confía el tenant cliente → consulta datos de B.
...
| TM-002 | Miembro tenant | ID o tenant ajeno | BOLA/claim falsificado | Fuga cross-tenant | tenant, datos | resolución server-side + RLS | matriz completa pendiente | authz objeto/acción/estado; 404 neutral | AccessDenied correlacionado | high | high | critical |
| TM-003 | Concurrencia/pool | setting persiste | Reutiliza contexto A | Fuga silenciosa | tenant, datos | transacción + `set_config local`; pool size 1 probado tras rollback/excepción | cancelación abrupta no probada | repetir stress y cancelación con pool real | mismatch tenant/correlación | medium | high | critical |
...
| TM-009 | Operador/config | ambiente mal configurado | Habilita fixture auth | Auth bypass | todos | fixture condicionado a Spike | deployment no existe | fail startup outside Spike; compile/exclude fixture | startup security event | low | high | high |
