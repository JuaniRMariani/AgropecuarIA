...
AgropecuarIA ya contiene un bootstrap productivo ejecutable local acotado: web Next.js, API ASP.NET Core, módulo Identity/Tenancy, PostgreSQL, OpenAPI, sesiones OIDC, linking, revocación, step-up MFA purpose-bound, journal local, outbox contractual, `CreateOrganization` idempotente e invitaciones one-shot de co-owner (`README.md`; `apps/AgropecuarIA.Api/Program.cs`; `tasks/evidence/AGRO-ID-003/product-decisions.md`). Estas fronteras tenant reproducen `ADR-PEND-007` mediante roles mínimos, contexto transaccional y `FORCE RLS` sobre organización, membresía, ledger e invitación; el discovery 0/1/N general continúa separado y pendiente de integración. No existe todavía despliegue compartido, edge, Auth0 real, worker de delivery, storage ni CI. El mayor riesgo continúa siendo extender aislamiento incompleto a nuevas rutas, jobs, caches o módulos. Los controles locales se consideran evidencia únicamente para los slices integrados y sus tests; nunca certifican proveedor, Legal, región, SLA o producción.
