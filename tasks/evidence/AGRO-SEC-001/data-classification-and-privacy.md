...
**Alcance:** baseline de decisión reconciliada con el runtime R1 local de Identity, Territory y Productive Core sobre API/web/PostgreSQL; no es dictamen legal, aviso de privacidad, aprobación de proveedor ni autorización de despliegue compartido/productivo.

## Niveles de evidencia

- **Runtime R1 local integrado:** `apps/AgropecuarIA.Api`, `src/AgropecuarIA.Identity`, `src/AgropecuarIA.Territory`, `src/AgropecuarIA.ProductiveCore`, `apps/web` y sus pruebas ejecutan autenticación, tenancy, referencia territorial y campos draft no espaciales contra PostgreSQL local. Es código de producto ejecutable, no evidencia de un entorno compartido.
...
- **Campos no espaciales:** Productive Core conserva `DisplayName` del campo como dato de negocio `Confidential`, junto con organization/resource UUID internos, estado draft, versión, revisión y fecha de creación. Este slice crea, lee y renombra el borrador, pero no acepta ni persiste coordenadas, área, geometría, tiles ni historial espacial. Las idempotency keys nunca se guardan en claro: los ledgers conservan fingerprint y aliases HMAC versionados; journal y outbox contienen UUID y atributos mínimos, nunca el nombre anterior o nuevo. El acceso queda limitado a owner activo revalidado antes de lookup/replay y a `FORCE RLS` tenant (`src/AgropecuarIA.ProductiveCore/Infrastructure/ProductiveCoreDbContext.cs`; `tests/AgropecuarIA.ProductiveCore.Tests/ProductiveCoreDatabaseSecurityTests.cs`).
...
- **Telemetría y auditoría:** Identity y Productive Core emiten localmente actividades/métricas OpenTelemetry con allow-list y dimensiones acotadas. Productive Core expone solo operación, outcome y conteo limitado; excluye nombre de campo, UUID de tenant/recurso, sesión e idempotencia. Territory registra solamente código de error y correlation ID; no query, coordenada ni payload provider. No hay exporter, collector, backend ni proxy compartido validado, por lo que `TST-OTEL-REDACTION` y `TST-TERRITORY-URL-REDACTION` siguen pendientes y bloquean egress real (`src/AgropecuarIA.ProductiveCore/ProductiveCoreTelemetry.cs`; `tests/AgropecuarIA.ProductiveCore.Tests/ProductiveCoreApplicationServiceTests.cs`).
...
| Productive Core — campos no espaciales | nombre de campo y pertenencia tenant; UUID internos, idempotencia y auditoría | owner activo revalidado server-side antes de lookup/replay; nombre `Confidential`; keys crudas nunca persistidas; HMAC versionado; rename con ETag/If-Match y 412 neutral; journal/outbox mínimos sin nombre; `SET LOCAL` + roles mínimos + `FORCE RLS`; sin coordenadas/área/geometría | GO solo integrado-local para field draft create/list/detail/rename. NO-GO geometría, roles no-owner, otras ediciones, delivery externo, DB/backup/secretos administrados, hosting compartido y producción |
