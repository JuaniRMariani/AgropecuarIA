...
|---|---|---|---|
| Identidad/tenancy | personal, sesión, account linking y acceso cruzado | runtime R1 local API/web/PostgreSQL con OIDC/PKCE, CSRF, rate limit, step-up e idempotent `CreateOrganization`; la frontera org integra roles mínimos, `SET LOCAL`, `FORCE RLS` y negativos A/B/sin contexto/pool/job; el spike `AGRO-DIS-003` conserva evidencia separada de discovery 0/1/N | GO local solo para el bootstrap de organización; NO-GO para discovery y nuevas mutaciones hasta repetir migraciones/principals/grants/negativos, y NO-GO IdP/DB/entorno compartido hasta credenciales administradas, sandbox, DPA/región/retención/subprocesadores/exportabilidad |
