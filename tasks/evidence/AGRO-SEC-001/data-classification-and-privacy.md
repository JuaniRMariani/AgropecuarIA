...
- **Campos no espaciales:** Productive Core conserva `DisplayName` del campo como dato de negocio `Confidential`, junto con organization/resource UUID internos, estado draft, versión y fecha de creación. Este slice no acepta ni persiste coordenadas, área, geometría, tiles ni historial espacial. La idempotency key nunca se guarda en claro: el ledger conserva fingerprint y aliases HMAC versionados; el journal/outbox contienen UUID y atributos mínimos del contrato, no nombre de campo. El acceso queda limitado a owner activo revalidado antes de lookup/replay y a `FORCE RLS` tenant (`src/AgropecuarIA.ProductiveCore/Infrastructure/ProductiveCoreDbContext.cs`; `tests/AgropecuarIA.ProductiveCore.Tests/ProductiveCoreDatabaseSecurityTests.cs`).
...
| Productive Core — campos no espaciales | nombre de campo y pertenencia tenant; UUID internos, idempotencia y auditoría | owner activo revalidado server-side antes de lookup/replay; nombre `Confidential`; key cruda nunca persistida; HMAC versionado; journal/outbox mínimos sin nombre; `SET LOCAL` + roles mínimos + `FORCE RLS`; sin coordenadas/área/geometría | GO solo integrado-local para field draft create/list/detail. NO-GO geometría, roles no-owner, delivery externo, DB/backup/secretos administrados, hosting compartido y producción |
