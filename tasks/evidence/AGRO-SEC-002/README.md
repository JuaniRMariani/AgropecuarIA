...
- Las 20 operaciones HTTP actuales de Identity y Territory coinciden exactamente entre OpenAPI, código de rutas y `authorization-surface-register.json`.
...
- Las operaciones tenant de invitaciones requieren tenant server-derived/revalidado, autorización owner, `FORCE RLS` y errores sin oracle cross-tenant.
...
Siguientes superficies tenant deben entrar al registro en el mismo cambio que su OpenAPI/runtime. El siguiente slice de producto sugerido sigue siendo revocación de co-owner con protección concurrente de último owner dentro de AGRO-ID-003.
