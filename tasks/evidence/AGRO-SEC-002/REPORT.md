...
No se confirmaron vulnerabilidades críticas, altas o medias explotables dentro del runtime local y la configuración default-off auditados. El registro vigente cubre 30/30 operaciones HTTP, incluido create/list/detail/rename de `ManagementUnit` en Productive Core e inventario/revocación individual/bulk/global de sesiones propias en Identity.
...
- La revocación bulk exige purpose exacto y CSRF, deriva actor/current en servidor, serializa por actor y sólo concede `EXECUTE` sobre una función que excluye current. La respuesta 204 omite IDs/count y los journals por target confirman o revierten junto con todo el batch.
