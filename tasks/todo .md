...
- [x] Preparar el puerto público de resolución versionada para ciclos; conectar después de fijar su contrato sin accesos de Productive Core a tablas privadas de Catalog. Los metadatos enviados por clientes nunca certifican soporte especializado. Implementación y criterios completados en Iteración 55; no cierra el padre CAT-003.
- [x] Actualizar contratos/fitness, probar E2E con fixtures claramente sintéticos, revisar independientemente y ejecutar gates.
- [x] Publicar el checkpoint verificado con la identidad Git solicitada e inspeccionar su CI remoto; su fallo y corrección posterior están documentados en el review.
...
- Publicado **`8f25ed64ba2eade2a36022702e9dce221276cb39`** (`feat(catalog): publish immutable versions and add authenticated reader`), author/committer **JuaniRMariani <juanirmariani@gmail.com>** verificados y hash remoto coincidente. Worktree limpio al finalizar el push. CI remoto de este checkpoint en inspección; implementación siguiente autorizada desde esta base local verificada.
- CI `33918321239` finalizó FAIL: frontend PASS, único método backend anotado `ConcurrentCreatesAtCapacityCommitExactlyOneCompleteSlice`. La prueba local original pasó, pero una barrera determinista (ambas solicitudes leen 99, la segunda guarda después del commit de la primera) reprodujo RED con `productive_core.management_unit_unavailable`. El interceptor confirmó `InvalidOperationException → DbUpdateException → PostgresException(40001)`: el clasificador inspeccionaba solo un nivel interno. Se recorre ahora la cadena conservando exclusivamente 40001/40P01, sin cambiar reintentos, unicidad ni commit desconocido. Regresión exacta GREEN 1/1 con 100 campos y una sola fila por ledger/alias/journal/outbox. Revisión independiente y gate integrado pendientes; no se declara CI remoto reparado hasta verificar un nuevo run.

## Iteración 55 — Referencia autoritativa de catálogo en ciclos

### Plan y aceptación técnica

- [x] Iniciar después de verificar/publicar el checkpoint Catalog de Iteración 54; no mezclar código a medio terminar con ese checkpoint.
- [x] Introducir un puerto propiedad de Productive Core y un adaptador en la composición API que consulte exclusivamente la superficie pública Application de Catalog. Sin ProjectReference Productive→Catalog, consultas a tablas privadas ni FK cruzada.
- [x] Versionar explícitamente el contrato Productive a 2.0.0 para inicio de ciclo: request estricto `{catalogCode,purpose,system,startDateUtc,expectedCatalogVersionId?}`. Rechazar metadatos de nombre/soporte enviados por cliente y propiedades desconocidas; documentar la migración del consumidor, no ocultar la incompatibilidad.
- [x] Autorizar organización/campo y rechazar archivo antes de consultar Catalog. Resolver activo, entrada y procedencia en un snapshot de lectura coherente. La versión esperada es una precondición del activo observado, no una selección de historia.
- [x] Congelar en el ciclo IDs de versión/entrada, etiqueta, código/nombre canónicos, soporte declarado, procedencia nullable y momento de resolución del servidor. Soporte efectivo nuevo siempre genérico, sin capacidades especializadas implícitas. Publicar después de la resolución puede cambiar el activo: el ciclo conserva la versión realmente observada, sin prometer atomicidad distribuida o activo al commit.
- [x] Agregar migración forward y guards de coherencia/inmutabilidad de referencias. Ciclos anteriores conservan texto/soporte originales con `legacy_unresolved` y snapshot null; no backfill por coincidencia del código actual. Una publicación legacy sí puede dar una referencia de versión real con procedencia de fuente no disponible.
- [x] Probar HTTP real y PostgreSQL: spoof/unknown/CSRF/owner/cross-tenant/archivo, catálogo ausente/stale/caído sin efectos, publicación concurrente después de resolver, lecturas históricas sin consultar Catalog y cierre sin alterar snapshot. Mantener ausencia de retries ante commit ambiguo.
- [x] Actualizar contratos/fitness/consumidores, revisar independientemente y ejecutar gates antes de publicar. Registrar los residuos del padre CAT-003: idempotencia/journal/eventos, outputs/costos/documentos, UI y certificación del baseline no se dan por completados por este puerto.

### Decisión

- Propuesta read-only del auditor contrastada con el código: `StartCycleAsync` aún acepta nombre y soporte del cliente; no existe cobertura HTTP real de `/cycles`. La entrega siguiente corrige esa autoridad y agrega esa cobertura, no renombra tests de metadata como integración. Plan aprobado técnicamente; implementación iniciada desde `8f25ed6`, con código backend y contratos/revisión en workstreams separados.

### Review en curso

- Implementación y revisión independiente del puerto/adapter, migración, HTTP y contratos completadas sin hallazgos abiertos. HTTP real **4/4**, resolución MVCC Catalog **2/2** y pruebas PG de owner/archivo y snapshot/legacy **4/4** ejecutadas por el agente. El cierre de ciclos se prueba como servicio, no se inventa una ruta HTTP de cierre.
- Los agentes se interrumpieron por límite de uso antes del gate integrado; root retomó la verificación. Restore locked/auditoría NuGet PASS; Release build **0W/0E** tras corregir un CA1861 en el nuevo test de arquitectura sin suprimir el analyzer. Web **346/346**, formato/lint/typegen/typecheck/build/auditoría PASS. Suite backend completa en curso.
...
- Corrección publicada como `389a88abc04e2b2af15484ab0f78d25ad2232dd4`, identidad Git y remoto coincidentes. CI **`33929041260` SUCCESS**, ambos jobs aprobados. El fallo de cupo ya tiene regresión determinista local y verificación remota con la corrección; no se atribuye retrospectivamente el primer fallo sin logs a la misma causa.

## Iteración 56 — Lecturas acotadas del historial de ciclos

### Plan y aceptación

- [x] Agregar endpoints GET paginados aditivos (`/fields/{fieldId}/cycles/page` y `/cycles/{cycleId}/timeline/page`) sin cambiar silenciosamente los envelopes 2.0 existentes. Documentar los GET anteriores como legado pendiente de retiro; este slice no certifica que todo el runtime tenga lecturas acotadas.
- [x] Aplicar límites en PostgreSQL: 20 por defecto, máximo 100, consulta de una fila adicional para `hasMore` y cursor de continuación contextual. Orden estable por fecha de registro descendente y UUID; índices correspondientes. No descargar todo para recortar en memoria.
...
- Endpoints aditivos Productive **2.1.0**, envelopes acotados y cursores contextuales implementados. Las consultas usan comparación SQL de filas `(timestamp, uuid)` y `LIMIT limit+1`; nueva migración añade índices sin modificar datos/grants. Los endpoints anteriores están deprecated, siguen ilimitados y no se afirma que la deuda esté cerrada.
- Prueba PostgreSQL con **105 ciclos y 105 observaciones** de fechas empatadas PASS: recorrido completo y orden exacto por UUID, bounds 20/100 y repositorio limitado a tres filas para límite dos. HTTP real PASS inicial y extendido con mutaciones de cursor, acceso y archivo; se agregó después cobertura de replay de cookie revocada y ausencia de recurso, pendiente del gate final. La matriz aislada usa cupo de 100 requests; producción conserva 30 y las pruebas dedicadas de rate limit no cambian.
