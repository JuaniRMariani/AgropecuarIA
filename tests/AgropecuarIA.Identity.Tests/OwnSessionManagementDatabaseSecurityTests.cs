...
public sealed class OwnSessionManagementDatabaseSecurityTests
{
    ...
    public async Task ConcurrentReplayProducesOneTransitionAndOneJournal()
    {
        ...
        Assert.AreEqual(1L, await ScalarInt64Async(
            ...
            WHERE "SessionId" = @target AND "Action" = 'session.revoke'
            """,
            ("target", seeded.TargetSessionId)));
    }
    ...
    private static async Task<SeededSessions> SeedAsync(string connectionString)
    {
        ...
        await using var command = new NpgsqlCommand(
            ...
            VALUES
                ...
                (gen_random_uuid(), @actor, @expiredHash, now() - interval '1 day',
                 now() - interval '1 hour', true, NULL, NULL, NULL, gen_random_uuid()),
                (gen_random_uuid(), @actor, @revokedHash, now() - interval '1 day',
                 now() + interval '1 hour', true, NULL, NULL, now(), gen_random_uuid()),
        ...
        SeededSessions seeded)
    {
        ...
        context.Parameters.AddWithValue("session", seeded.CurrentSessionId.ToString("D"));
        context.Parameters.AddWithValue("version", seeded.AuthorizationVersion.ToString("D"));
        ...
        Guid newVersion)
    {
        ...
        if (outcome == "revoked")
        {
            await using var journal = new NpgsqlCommand(
                ...
                VALUES (@id, @actor, @target, 'session.revoke', 'succeeded',
